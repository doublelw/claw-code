...
impl ConfigLoader {
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        let user_legacy_path = self.config_home.parent().map_or_else(
            || PathBuf::from(".clawc.json"),
            |parent| parent.join(".clawc.json"),
        );
        vec![
            ConfigEntry {
            ...
            ConfigEntry {
                ...
                path: self.cwd.join(".clawc.json"),
            },
            ConfigEntry {
                source: ConfigSource::Project,
                path: self.cwd.join(".clawc").join("settings.json"),
            },
            ConfigEntry {
                source: ConfigSource::Local,
                path: self.cwd.join(".clawc").join("settings.local.json"),
            },
            // Pre-rename `.claw` layout fallback (read-only): existing
            // installations keep loading until they migrate to `.clawc`.
            ConfigEntry {
                source: ConfigSource::Project,
...
pub fn default_config_home() -> PathBuf {
    std::env::var_os("CLAW_CONFIG_HOME")
        .map(PathBuf::from)
        .or_else(|| std::env::var_os("HOME").map(|home| PathBuf::from(home).join(".clawc")))
        .unwrap_or_else(|| PathBuf::from(".clawc"))
}

/// Save provider settings to the user-level `~/.clawc/settings.json`.
...
/// Remove the `provider` section from the user-level `~/.clawc/settings.json`.
...
fn read_optional_json_object(path: &Path) -> Result<Option<ParsedConfigFile>, ConfigError> {
    let is_legacy_config = path.file_name().and_then(|name| name.to_str()) == Some(".clawc.json");
...
mod tests {
    ...
    fn loads_and_merges_claude_code_config_files_by_precedence() {
        ...
        let cwd = root.join("project");
        let home = root.join("home").join(".clawc");
        fs::create_dir_all(cwd.join(".clawc")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");

        fs::write(
            home.parent().expect("home parent").join(".clawc.json"),
    ...
    fn rejects_invalid_hook_entries_before_merge() {
        ...
        let cwd = root.join("project");
        let home = root.join("home").join(".clawc");
        let project_settings = cwd.join(".clawc").join("settings.json");
        fs::create_dir_all(cwd.join(".clawc")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");
