...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let claw_dir = cwd.join(".clawc");
    artifacts.push(InitArtifact {
        name: ".clawc/",
        status: ensure_dir(&claw_dir)?,
    });

    let claw_json = cwd.join(".clawc.json");
    artifacts.push(InitArtifact {
        name: ".clawc.json",
...
pub(crate) fn render_init_claude_md(cwd: &Path) -> String {
    ...
    lines.push("- Keep shared defaults in `.clawc.json`; reserve `.clawc/settings.local.json` for machine-local overrides.".to_string());
...
mod tests {
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(rendered.contains(".clawc/"));
        assert!(rendered.contains(".clawc.json"));
        assert!(rendered.contains("created"));
        assert!(rendered.contains(".gitignore       created"));
        assert!(rendered.contains("CLAUDE.md        created"));
        assert!(root.join(".clawc").is_dir());
        assert!(root.join(".clawc.json").is_file());
        assert!(root.join("CLAUDE.md").is_file());
        assert_eq!(
            fs::read_to_string(root.join(".clawc.json")).expect("read claw json"),
            concat!(
                "{\n",
                "  \"permissions\": {\n",
                "    \"defaultMode\": \"dontAsk\"\n",
        ...
        assert!(gitignore.contains(".clawc/settings.local.json"));
        assert!(gitignore.contains(".clawc/sessions/"));
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        fs::write(root.join(".gitignore"), ".clawc/settings.local.json\n")
        ...
        assert!(second_rendered.contains(".clawc/"));
        assert!(second_rendered.contains(".clawc.json"));
        ...
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
        assert_eq!(gitignore.matches(".clawc/settings.local.json").count(), 1);
        assert_eq!(gitignore.matches(".clawc/sessions/").count(), 1);
