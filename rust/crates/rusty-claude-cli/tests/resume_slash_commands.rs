...
fn resumed_config_command_loads_settings_files_end_to_end() {
    ...
    fs::create_dir_all(project_dir.join(".clawc")).expect("project config dir should exist");
    ...
    fs::write(
        project_dir.join(".clawc").join("settings.local.json"),
        r#"{"model":"opus"}"#,
    )
...
fn resumed_status_command_emits_structured_json_when_requested() {
    ...
    // when
    // Use an isolated CLAW_CONFIG_HOME so ~/.clawc/settings.json is not loaded,
